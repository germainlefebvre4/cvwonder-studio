...
Le système SHALL permettre aux utilisateurs connectés de créer jusqu'à la limite définie par la clé `max_sessions_per_user` de `system_config` (défaut : 10). Le backend SHALL refuser la création si le nombre de sessions actives non archivées atteint cette limite.

#### Scenario: Création dans la limite du quota
- **WHEN** un utilisateur connecté ayant 7 sessions sur 10 crée une nouvelle session
- **THEN** la session est créée et liée à son `user_id`

#### Scenario: Dépassement du quota
- **WHEN** un utilisateur connecté ayant atteint sa limite tente de créer une session
- **THEN** le backend retourne HTTP 422 avec un message indiquant la limite atteinte et suggérant d'archiver des sessions existantes
