...
The `infra/k8s/` directory SHALL contain `pvc-sessions.yaml` and `pvc-themes.yaml` with `accessModes: ReadWriteOnce` and configurable storage size. The Deployment SHALL mount:
- `pvc-sessions` at `/data/sessions/` (where generated CVs are stored)
- `pvc-themes` at `/data/themes/` (where runtime-installed themes are stored)
